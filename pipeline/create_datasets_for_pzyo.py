...
def get_solidity_distilled_data(data_path):
    ...
    output_file = f"{data_path}/distilled_solidity_data_pzyo.json"
    with open(output_file, "w") as file:
        ...
        for patch_id, patch_data in tqdm(all_results_train.items(), total=len(all_results_train)):
            ...
            llm_comment = max_item.get("llm_comment", "").strip()
            ...
            sol_max = max_item.get("patch", "").strip()
            sol_min = min_item.get("patch", "").strip()
            ...
            if total_max_gas != 0 and (total_max_gas - total_min_gas) / total_max_gas < 0.0001:
                continue
            ...
            sample_dict = {
                ...
                "ratio": ratio,
                # gas / vulnerability for DPO custom loss
                "gas_chosen": total_min_gas,
                "gas_rejected": total_max_gas,
                "vul_chosen": min_item.get("vulnerability", False),
                "vul_rejected": max_item.get("vulnerability", False),
            }
...
def process_data_for_sft(sample):  
    messages = [
        ...
        {
            "role": "assistant", 
            "content": sample["response_k"].strip()
        },
    ]
...
def process_data_for_dpo(sample):
    return {
        ...
        "rejected": [
            ...
            {
                "role": "assistant",
                "content": sample["response_j"].strip(),
            }
        ],
    }

def process_data_for_pzyo(sample):
    return {
        "prompt": sample["prompt"].strip(),
        "chosen": [
            { 
        ...
        "gas_chosen": sample["gas_chosen"],
        "gas_rejected": sample["gas_rejected"],
        "vul_chosen": sample["vul_chosen"],
        "vul_rejected": sample["vul_rejected"],
    }
    

def process(num_proc: int, data_path: str, post_training_type: str):
    """
    此函数加载 distilled 数据（针对 Solidity 数据集使用 distilled_solidity_data_pzyo.json），
    并基于不同后训练类型（sft, dpo, grpo）对数据进行包装，同时保留原始所有 key-value 信息，
    然后对数据进行 train/test 切分，并将两个 split 分别以 JSON 格式保存。
    """

    dataset = load_dataset('json', data_files=f"{data_path}/distilled_solidity_data_pzyo.json")["train"]
    ...
    elif post_training_type == "pzyo":
        process_example_map = partial(process_data_for_pzyo)
...
if __name__ == "__main__":
    ...
    for post_training_type in ["pzyo"]:        
        cache_dir = f"./datasets/{post_training_type}_solidity_data"
